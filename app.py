...
Two modes, chosen in the sidebar:
  1. Fill out survey  -> writes one row to Supabase (survey_responses)
...
st.set_page_config(page_title="Partner Needs Assessment", page_icon="📋", layout="wide")
...
def render_survey():
    ...
    st.caption("Digital Health, Data & AI Capacity Baseline — takes about 15–20 minutes.")
    ...
    with st.form(key=f"survey_form_{k}"):
        ...
        institution_name = st.text_input("Institution name", key=f"a1_{k}")
        ...
        staff_count = st.selectbox(
            "Number of staff working on digital health / data / AI-related activities (approx.)",
            STAFF_BUCKETS,
            key=f"a5_{k}",
        )
        ...
        q1_systems_count = st.selectbox(
            "How many separate digital systems does your institution currently use for "
            "clinical, research, surveillance, or knowledge-management purposes?",
            COUNT_BUCKETS_0TO6PLUS,
            key=f"q1_{k}",
        )
        q2_integration_rating = rating_radio(
            "To what extent do these systems share data with each other automatically "
            "(without manual re-entry)?",
            key=f"q2_{k}",
        )
        q3_coordination = st.selectbox(
            "Is there a single unit or person responsible for coordinating digital/data "
            "systems across your institution?",
        ...
        q4_governance_policy = st.selectbox(
            "Does your institution have a formal, written data governance policy "
            "(ownership, access rights, sharing conditions)?",
            ["Yes, documented & applied", "Yes, but inconsistently applied", "Being developed", "No"],
            key=f"q4_{k}",
        )
        q5_standards = st.multiselect(
            "Which data/interoperability standards does your institution currently use?",
            STANDARDS_OPTIONS,
            key=f"q5_{k}",
        )
        q6_data_sharing_rating = rating_radio(
            "Rate your institution's ability to securely share data with external partners "
        ...
        q7_ai_use = st.selectbox(
            "Does your institution currently use any AI-enabled tools (diagnostic support, "
            "triage, predictive analytics, image analysis, etc.)?",
            ["Yes, routine use", "Yes, pilot/testing", "No, but planned", "No"],
            key=f"q7_{k}",
        )
        q8_bias_audit = st.selectbox(
            "Has any AI tool used at your institution undergone a formal bias/fairness or "
            "performance audit before deployment?",
            ["Yes", "No", "Not applicable", "Don't know"],
            key=f"q8_{k}",
        )
        q9_ai_confidence_rating = rating_radio(
            "Rate your confidence in your institution's ability to check whether a "
            "digital/AI tool performs reliably for your specific population, language, "
            "and infrastructure.",
            key=f"q9_{k}",
        )
        ...
        q10_trained_staff = st.selectbox(
            "How many staff at your institution have formal training in digital health, "
            "health informatics, bioinformatics, or AI?",
            COUNT_BUCKETS_0TO10PLUS,
            key=f"q10_{k}",
        )
        q11_staffing_rating = rating_radio(
            "Rate the adequacy of current staffing to design, run, and maintain digital "
            "health/data/AI initiatives.",
            key=f"q11_{k}",
        )
        ...
        q13_pilot_not_scaled = st.selectbox(
            "Has your institution ever piloted a digital health/data solution that was NOT "
            "later adopted at wider scale (institutional, regional, or national)?",
        ...
        q15_scaleup_readiness = rating_radio(
            "Rate your institution's current readiness to move a validated pilot into "
            "routine, wider-scale operational use.",
            key=f"q15_{k}",
        )
        ...
        q16_policy_link = st.selectbox(
            "Does your institution have an active link — formal or informal — with "
            "national health/digital policy bodies (Ministry of Health, digital health "
            "agency, regulator)?",
            ["Yes, formal", "Yes, informal", "No", "Don't know"],
            key=f"q16_{k}",
        )
        q17_policy_feed_rating = rating_radio(
            "Rate the extent to which your institution's digital health work currently "
            "feeds into national strategy or policy decisions.",
            key=f"q17_{k}",
        )
        q18_financing_mechanism = st.selectbox(
            "Does your institution have a defined financing/budget mechanism to sustain "
            "digital tools once donor or project funding ends?",
        ...
        q19_infrastructure_rating = rating_radio(
            "Rate the reliability of internet connectivity and electricity supply at your "
            "main site(s).",
            key=f"q19_{k}",
        )
        q20_gender_policy = st.selectbox(
            "Does your institution have a Gender Equality Plan (or equivalent inclusion/"
            "equity policy)?",
            ["Yes", "In development", "No", "Don't know"],
            key=f"q20_{k}",
        )
        q21_leadership_rating = rating_radio(
            "Rate senior leadership's active support (visible commitment, resources, time) "
            "for digital health/data initiatives.",
            key=f"q21_{k}",
        )
        q22_eu_projects_count = st.selectbox(
            "How many international (EU/Horizon Europe or equivalent) research/innovation "
            "projects has your institution participated in over the past 5 years?",
            COUNT_BUCKETS_EU_PROJECTS,
            key=f"q22_{k}",
        )
        q23_dissemination_rating = rating_radio(
            "Rate your institution's capacity to disseminate results (website, social "
            "media, publications, policy briefs).",
            key=f"q23_{k}",
        )
        q24_grant_mgmt_rating = rating_radio(
            "Rate your institution's experience managing multi-partner EU/international "
        ...
        q25_biggest_need = st.text_area(
            "What is the single biggest need your institution has to improve its digital "
            "health / data / AI capacity?",
            key=f"q25_{k}",
        )
        q26_valuable_support = st.text_area(
            "What kind of support from this consortium (training, mentoring, funding, "
            "equipment, connections) would be most valuable to you?",
            key=f"q26_{k}",
        )
    ...
    if submitted:
        if not institution_name.strip():
            st.error("Please enter your institution name before submitting.")
            return
...
def render_dashboard():
    ...
    cat_cols = {
        "q7_ai_use": "AI tool use",
        "q13_pilot_not_scaled": "Pilots not scaled up",
        "q16_policy_link": "Link with national policy bodies",
